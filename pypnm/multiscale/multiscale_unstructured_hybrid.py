...
from pypnm.attribute_calculators.pc_computer import DynamicCapillaryPressureComputer, CapillaryPressureComputer
from pypnm.flow_simulation.dynamic_simulation import DynamicSimulation
from pypnm.flow_simulation.dynamic_to_quasi_simulation import QuasiStaticSimulation
...
class MultiScaleSimUnstructured(MultiscaleSim):
    ...
    def __init__(self, network,  fluid_properties, num_subnetworks, comm=None, mpicomm=None, subgraph_ids=None,
        ...
        # Create dynamic and quasistatic simulations
        self.simulations = dict()

        for i in self.my_subgraph_ids:
            self.q_influx = -1   #I added this

            if i % 2 == 0 and self.q_influx > 0:
                self.simulations[i] = QuasiStaticSimulation(self.my_subnetworks[i],
                                                            self.fluid_properties)  # I added this

                k_comp = ConductanceCalc(self.my_subnetworks[i], self.fluid_properties)
                k_comp.compute()
                pc_comp = CapillaryPressureComputer(self.my_subnetworks[i])
                pc_comp.compute()
            else:
    ...
    def advance_in_time(self, delta_t):
        ...
        while True:
                ...
                for i in self.my_subgraph_ids: # nonwetting fluid source/sink
                    self.q_influx = np.sum([bc[i].pi_list_nw_source])



                for i in self.simulations:
                    if (i % 2 == 0 and self.q_influx>0) == False:
                ...
                for i in self.simulations:
                    if i%2!=0 and self.q_influx > 0:
                        self.simulations[i] = QuasiStaticSimulation(self.my_subnetworks[i], self.fluid_properties)
                        self.simulations[i].apply_current_conditions()
                        ...
                        # saturation manipulation for adaptive pnm


                        print('flux for', i, 'is', self.q_influx)
                        #k_comp = ConductanceCalc(self.my_subnetworks[i], self.fluid_properties)

                        #try:
                        #    q = flux_into_pores(self.network, press_bc_inlet ,k_comp.compute(), self.my_subnetworks[i].pi_in)
                        #except:
                         #   q = flux_from_into_pores(self.network, press_bc_inlet ,k_comp.compute(), self.my_subnetworks[i-1].pi_out, self.my_subnetworks[i].pi_in)

                        total_volume = np.sum(self.my_subnetworks[i].tubes.vol) + np.sum(self.my_subnetworks[i].pores.vol)
                        sat_change = ((self.q_influx*1.e-11) * dt) / total_volume
                        subnetwork_saturation = self.simulations[i].sat_comp.sat_nw()
                        initial_sat = subnetwork_saturation
                        final_sat = initial_sat + sat_change
                        assert final_sat <= 0.9999999

                        saturation_points = np.linspace(initial_sat, final_sat)
                        # saturation_points = np.concatenate([np.linspace(initial_sat,final_sat),np.linspace(final_sat,initial_sat)])
                        ...
                        for n, sat in enumerate(saturation_points):
                            ...
                            if (n > 0) & (saturation_points[n] > saturation_points[n - 1]):
                                if self.simulations[i].sat_comp.sat_nw_conn() < saturation_points[n]:
                                    self.simulations[i].update_saturation_conn(sat)
                                else:
                                    continue
                            ...
                            sat_nw_conn = self.simulations[i].get_nonwetting_connected_saturation()
                            pc_qs = self.my_subnetworks[i].pores.p_c[self.my_subnetworks[i].pores.connected == 1]
                            #p_c_max = np.max(pc_qs)
